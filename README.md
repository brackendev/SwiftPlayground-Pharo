Because of instability with Xcode's Swift Playgrounds, this robust Swift code runner was implemented for Pharo. It brings the benefits of working with Swift output in the powerful Pharo environment. This is a work in progress.
...
For example, in the Swift code below, an Apple [Foundation](https://developer.apple.com/documentation/foundation) [URLRequest](https://developer.apple.com/documentation/foundation/urlrequest) network request session is started and then followed by `dispatchMain()` so the program does not prematurely exit. In the session response closure `exit(0)` is then called to exit the program.
